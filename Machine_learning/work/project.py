...
st.markdown("선형 회귀 기반으로 캘리포니아의 집값을 예측합니다.")
...
# alpha 관련 슬라이더 및 자동 선택 옵션
auto_alpha = False
alpha = None
if model_name != "Linear Regression":
    auto_alpha = st.sidebar.checkbox("📈 alpha 자동 탐색 (GridSearch)", value=False)
    if not auto_alpha:
        alpha = st.sidebar.slider("정규화 강도 (alpha)", 0.01, 10.0, 1.0)
...
if model_name == "Linear Regression":
    ...
    if auto_alpha:
        # 자동 탐색
        model_class = Ridge if model_name == "Ridge Regression" else Lasso
        alpha_range = np.logspace(-2, 1, 20)
        grid = GridSearchCV(
            model_class(),
        ...
        st.sidebar.success(f"✔ 최적 alpha: {best_alpha:.4f}")
    else:
        model = (
            Ridge(alpha=alpha)
            if model_name == "Ridge Regression"
            else Lasso(alpha=alpha)
        )
