...
def generate_title_from_recipes(response_text):
    ...
    messages = [
        {
            "role": "system",
            "content": "다음 대화 내용을 보고 10자 이내의 한국어 제목을 만들어주세요. 너무 일반적이거나 추상적인 표현은 피하고, 대화 주제를 압축해서 표현해주세요.",
        },
    ...
    assistant_title = response.choices[0].message.content
...
def generate_conversation(user_input):
    ...
    character_prompt = characters[character][0]
...
# ============================
# json 파싱 -> openai api 연동
# stream = true 로 설정은했으나 답변이 전부 생성되면 출력하는 현상 수정 필요
